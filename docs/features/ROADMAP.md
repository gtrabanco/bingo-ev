...
| 11 | `bidirectional-device-transfer` | planned | — | **S.** Make the device-code transfer bidirectional. Current flow: source device generates a code/QR → destination device claims it → destination loads source's card. New behaviour: if the claiming device already has an active card in localStorage, the flow inverts — the source device (the one that generated the code) loads the claiming device's card instead. This covers the Tesla use-case where the Tesla browser opens /activar, the phone scans the QR on-screen, and the Tesla loads the phone's card (rather than the phone loading the Tesla's non-existent card). Requires the claiming endpoint to detect whether the claimer has a card and, if so, push that card's id+secret back to the generating device via a short-lived result slot that the polling QR screen reads. |
