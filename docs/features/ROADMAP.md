...
| 14 | `design-refactor-design-system` | done | — | **L.** Site-wide visual refactor applying the pre-authored design system in `14-design-refactor-design-system/integration/` (self-hosted Bricolage/Lora/Space Mono, `SiteNav` icon-over-label navbar, cartón without "BINGO", cartón-protagonist home). CSS/fonts/markup/copy **only** — game behaviour, identity model, and **every** DOM/script hook preserved verbatim (verified: empty `comm -23` of old-nav vs SiteNav ids). Owner decisions: whole-site incl. all 9 secondary pages (D2), diploma **and** OG images adopt real fonts (D3), home restructure included (D4). Delivered as **8 independently-shippable slices** S1–S8 on a single branch (one gate-green commit per slice, single PR after S8 — D10): S1 fonts+`global.css`+preload+doc-convention, S2 drop-BINGO, S3 SiteNav swap, S4 home protagonist, S5 shared `<CardFrame>`+bespoke `c`/`v`/`g`, S6 light pages+site-wide nav, S7 diploma canvas fonts, S8 OG subset fonts. Fonts self-hosted woff2 (offline-converted, **no runtime dep** — D1/D8). Not issue-born — GitHub issue #14 is unrelated, **no** `Closes #14` (D9). SPEC: `14-design-refactor-design-system/`. |
| 16 | `home-share-images` | done | — | **M.** Two share images for the home, both mirroring the proven diploma image pattern (SVG generator → CF Image Resizing PNG, SVG fallback). **P1 — landscape 1200×630** `/og/home.png` as `og:image` (link previews: X/FB/WhatsApp/LinkedIn/Telegram + IG Stories-sticker/DM); redesigns `homeSvg()` into a real bingo card with readable situation text + hook. **P2 — portrait 1080×1920** `/og/home-story.png` as an uploadable Story/Reel/TikTok graphic (the formats that render no link preview). Both deterministic, two-tier safe zone. `Layout.astro` home default flips to PNG. SPEC: `16-home-share-images/`. |
| 17 | `server-side-account` | planned | — | **S.** Eliminate the client-side `GET /api/account` fetch (causes a browser-logged 401 on every page load for unauthenticated visitors). Read session + account in `index.astro` frontmatter; inline as JSON; extract `AccountBar.astro` with `server:defer` for correct server-rendered initial state without FOUC. Two phases: P1 inline JSON + remove client fetch, P2 AccountBar component. SPEC: `17-server-side-account/`. |
| 15 | `astro-7-upgrade` | done | — | **S.** Toolchain bump: `astro` 6.4.6→**7.0.0** (Rust compiler + Vite 8 defaults), `@astrojs/cloudflare` 13.7.0→**14.0.0**, `wrangler` 4.100.0→4.103.0; Tailwind/uqr/newsletter already latest. Verified compatible: astro 7 needs Node ≥22.12 (already required), pulls Vite 8; adapter 14 peers astro ^7 + wrangler ^4.83; `@tailwindcss/vite` already Vite-8 ready. Config audited as low-risk (no experimental flags, no view transitions, no content collections/md, no `src/fetch.ts`, no `@astrojs/db`, plain `cloudflare()` adapter — none of v7/v14's removals apply). Real risk is only the Rust compiler's stricter HTML (build-gated) + the new `compressHTML:'jsx'` whitespace default → **pin `compressHTML: true`** to preserve v6 output (D1). No schema/behaviour/copy change. Bundles version-string doc updates (D2), single PR, no `Closes #N` (D3). SPEC: `15-astro-7-upgrade/`. |
