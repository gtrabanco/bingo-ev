...
| 11 | `hall-of-fame` | in-progress | — | **S.** Top navigation bar consolidating four affordances on the game page: the **Hall of Fame** link, **Google/X login** buttons, the **logged-in user** chip (label + logout + profile control), and a **device-transfer** entry point. Slice 1 shipped (rename `/galeria` → `/hall-of-fame` with 301 redirect + bare nav link, PR #22); this completes the navbar by relocating the account bar and device-transfer control into it. UI-only, no schema, all element ids preserved. Soft dep on PR #21 (null-safe login-button refs). SPEC: `11-hall-of-fame/` |
| 12 | `bidirectional-device-transfer` | planned | — | **S.** Make the device-code transfer bidirectional. Current flow: source device generates a code/QR → destination device claims it → destination loads source's card. New behaviour: if the claiming device already has an active card in localStorage, the flow inverts — the source device loads the claiming device's card instead. Covers the Tesla use-case: Tesla opens /activar, phone scans QR on screen, Tesla loads phone's card. Requires a short-lived result slot that the QR-screen polls after the claimer responds. |
