...
- **Diploma lifecycle** (D3/D4 — `decisions.md`):
  ...
  - Completed cards are retained for **12 months** from `completed_at`, then swept by the
    opportunistic GC that runs on each `POST /api/cards` (same batch as the incomplete
    expiry sweep). Grouped cards settle departure before deletion.
